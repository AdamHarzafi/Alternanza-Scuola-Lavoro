(function () {
    ...
    function mount(db, user) {
        ...
        root.innerHTML = `<section class="teacher-hero" aria-labelledby="teacher-title"><div class="teacher-hero-copy"><p class="eyebrow">Registro docenti · FSL</p><h1 id="teacher-title">La classe,<br><span>a colpo d’occhio.</span></h1><p>Ogni percorso ha la sua storia. Qui trovi le ore e le esperienze di tutti gli studenti, sempre aggiornate.</p><div class="teacher-metrics" aria-label="Riepilogo della classe"></div></div><img class="teacher-hero-image" src="IMMAGINI/DASHBOARD-STUDENTI-PER-DOCENTI.png" alt="Tutti i tuoi studenti" width="1080" height="1080" fetchpriority="high" decoding="async"></section><section id="classe" aria-labelledby="class-title"><div class="teacher-section-head"><div><p class="eyebrow">I percorsi degli studenti</p><h2 id="class-title">Tutti, in un unico posto.</h2></div><button type="button" class="teacher-export" disabled>Esporta resoconto</button></div><div class="teacher-toolbar"><label class="teacher-search">Cerca uno studente<input type="search" placeholder="Cerca per nome…" autocomplete="off"></label><label>Mostra<select data-filter><option value="all">Tutti gli studenti</option><option value="deleted">Con attività eliminate</option><option value="modified">Con attività modificate</option><option value="unlinked">Account da collegare</option></select></label><label>Ordina per<select data-sort><option value="name">Nome, A–Z</option><option value="low">Ore crescenti</option><option value="high">Ore decrescenti</option></select></label></div><p class="teacher-status" role="status" aria-live="polite">Caricamento dei registri…</p><div class="student-grid"></div><p class="teacher-footnote">Le ore comprendono le attività predefinite e quelle personali. Le esperienze eliminate restano consultabili e sono escluse dai totali. Il registro è un riepilogo: la validazione ufficiale spetta all’istituto.</p></section>`;
        ...
        detail.innerHTML = `<div class="student-detail-scroll" tabindex="0" role="region" aria-label="Esperienze dello studente"><div class="student-detail-head"><div><p class="eyebrow">Registro studente</p><h2 id="student-detail-title"></h2></div><button class="student-close" type="button" aria-label="Chiudi registro studente">×</button></div><div class="student-detail-summary"></div><div class="student-detail-controls"><div class="student-tabs" role="group" aria-label="Stato delle esperienze"><button type="button" data-state="active" aria-pressed="true">Attive</button><button type="button" data-state="deleted" aria-pressed="false">Eliminate</button></div><label class="sr-only" for="teacher-category">Categoria</label><select id="teacher-category"><option value="all">Tutte le categorie</option><option value="formazione">Formazione scolastica</option><option value="certificazioni">Esperienza con attestazione</option></select><button type="button" class="detail-export">Esporta</button></div><p class="detail-status" role="status"></p><div class="student-experiences"></div></div>`;
        ...
        function openStudent(id) { selected = id; selectedState = 'active'; detail.querySelector('select').value = 'all'; renderDetail(); detail.showModal(); detail.querySelector('.student-detail-scroll').scrollTop = 0; document.documentElement.classList.add('register-modal-open'); }
