...
window.addEventListener('load', () => {
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            // Rimosso const HID_VALIDO (non serve più, ora controlla il database)
...
document.getElementById('hid-submit-btn').addEventListener('click', async () => { 
    ...
    if (typeof window.db !== 'undefined') {
        // Cerca l'HID nella collezione "studenti"
        window.db.collection("studenti").where("HID", "==", inputHidVal).get().then((snap) => {
            if (!snap.empty) {
                // HID trovato nel database!
                const userData = snap.docs[0].data();
                hidModal.classList.remove('active'); 
                submitBtn.innerHTML = originalBtnText;
                submitBtn.disabled = false;
                document.getElementById('hid-input').value = ""; // Pulisce il campo
                entraNelPortale(userData.nome); // Entra col vero nome dell'utente
            } else {
                // HID non trovato
        ...
        submitBtn.disabled = false;
            ...
            function entraNelPortale(nomeUtente) {
                ...
                const landing = document.getElementById('landing-view'); const dash = document.getElementById('app-dashboard');
                isScrolled = false; navbar.classList.remove('scrolled'); landing.style.opacity = '0'; window.scrollTo({ top: 0, behavior: 'smooth' });
                
                setTimeout(() => { 
                    landing.style.display = 'none'; dash.style.display = 'block'; void dash.offsetWidth; dash.style.opacity = '1'; 
            ...
            // ==========================================
            // LOGICA UNIFICATA RECUPERO PASSWORD (OTP & CIE)
            // ==========================================
